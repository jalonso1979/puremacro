...
Whatever the policy, a `('schema_drift', ...)` event is recorded in `connector_events` — with `fallback_used` `'none'`, `'warning_emitted'` or `'ignored'` respectively, and subject to the telemetry kill-switch of Section 7 — so `connector_health()` sees the drift even when a fallback hid it from the caller. A policy string outside `DRIFT_POLICIES` raises `ValueError` on the first call, before the payload is looked at, rather than months later inside a fallback path.
...
`load_realtime_cartridge(path, verify=True)` reads the zip through `pocket.load`, which with `verify=True` (the default) compares every stored payload's digest with the manifest and raises `pocket.CartridgeError` on a mismatch. `verify=False` skips that comparison altogether, so a corrupted frame loads silently: turn it off only on a large cartridge you already trust. It returns a `VintagePanel` whose `metadata` carries `cartridge_path`, `provenance_source`, `provenance_vintage` and `provenance_notes`. The digest depends on the data alone, not on the clock, so `pocket.load(path).verify()` can re-encode the loaded frames and compare again — which also catches a frame mutated in memory since the load — but `load_realtime_cartridge` does not call it for you.
...
- **Telemetry.** Events use outcomes `success`, `fallback` and `schema_drift`, with `fallback_used` values `none`, `sqlite_cache`, `warning_emitted` and `ignored`. No event is written for the no-credential store path or for a cold-store raise. The connectors write through `_cache_db.record_connector_event`, which honors the `PUREMACRO_NARRATIVE_TELEMETRY=0` kill-switch of [CONNECTOR_HEALTH.md](CONNECTOR_HEALTH.md) and becomes a no-op when it is set, so with telemetry off no connector event reaches `connector_events` at all. `connector_health()` counts every row with `fallback_used != 'live'` as a fallback, so a successful real-time fetch (`'none'`) is counted in both `n_success` and `n_fallback`; query `connector_events` directly for these sources.
