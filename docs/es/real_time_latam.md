...
Sea cual sea la política, se registra un evento `('schema_drift', ...)` en `connector_events` —con `fallback_used` igual a `'none'`, `'warning_emitted'` o `'ignored'`, respectivamente, y sujeto al interruptor de emergencia de telemetría de la sección 7—, de modo que `connector_health()` ve la deriva incluso cuando un respaldo se la ocultó a quien llamó. Una cadena de política fuera de `DRIFT_POLICIES` lanza `ValueError` en la primera llamada, antes de mirar la carga, y no meses después dentro de una ruta de respaldo.
...
`load_realtime_cartridge(path, verify=True)` lee el zip mediante `pocket.load`, que con `verify=True` (el valor por defecto) compara el resumen (*digest*) de cada carga almacenada con el manifiesto y lanza `pocket.CartridgeError` ante una discrepancia. `verify=False` omite esa comparación por completo, de modo que una tabla corrupta se carga en silencio: desactívelo solo en un cartucho grande en el que ya confíe. Devuelve un `VintagePanel` cuya `metadata` lleva `cartridge_path`, `provenance_source`, `provenance_vintage` y `provenance_notes`. El resumen depende solo de los datos, no del reloj, así que `pocket.load(path).verify()` puede volver a codificar las tablas cargadas y compararlas de nuevo —lo que además detecta una tabla mutada en memoria desde la carga—, pero `load_realtime_cartridge` no lo llama por usted.
...
- **Telemetría.** Los eventos usan los resultados `success`, `fallback` y `schema_drift`, con los valores de `fallback_used` `none`, `sqlite_cache`, `warning_emitted` e `ignored`. No se escribe ningún evento para la ruta del almacén sin credenciales ni para una excepción con el almacén frío. Los conectores escriben a través de `_cache_db.record_connector_event`, que respeta el interruptor de emergencia `PUREMACRO_NARRATIVE_TELEMETRY=0` de [CONNECTOR_HEALTH.md](CONNECTOR_HEALTH.md) y se convierte en una operación nula cuando está activado, así que con la telemetría desactivada ningún evento de los conectores llega a `connector_events`. `connector_health()` cuenta como respaldo toda fila con `fallback_used != 'live'`, de modo que una descarga en tiempo real exitosa (`'none'`) se cuenta tanto en `n_success` como en `n_fallback`; consulte `connector_events` directamente para estas fuentes.
...
- **Los cartuchos** son un formato de transporte: el SHA-256 no está firmado, `verify=False` omite por completo la comparación de resúmenes al leer (solo el valor por defecto `verify=True` la comprueba), y el archivo solo puede contener lo que el códec npz acepta (ningún objeto arbitrario de Python). Como señala el módulo `pocket`, las funciones de descarga necesitan sockets, así que en una sesión de Pyodide estos conectores solo pueden servir la caché o un cartucho — el cartucho es la vía de entrada prevista ([CACHE_DB.md](CACHE_DB.md) para la advertencia sobre la persistencia en IDBFS).
