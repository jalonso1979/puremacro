...
- **Las secciones 1 a 5 de esta página, y los proveedores de tiempo real respaldados por un archivo histórico** (`oecd_stes`, `alfred`, `bundesbank`, `ons`, `statcan`, `ecb_rtd`) operan a través de `puremacro.fetch._http.cached_get`, de modo que heredan la caché HTTP compartida, la sustitución del User-Agent, el respaldo SSL y el tiempo de espera de 30 segundos; toda URL que consultan aparece en `puremacro.cache.http_list_urls()`.
