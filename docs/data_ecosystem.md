...
- **Sections 1-5 below, and the archive-backed real-time providers** (`oecd_stes`, `alfred`, `bundesbank`, `ons`, `statcan`, `ecb_rtd`) go through shared, cached HTTP helpers — two of them. The section 1-5 collectors call `puremacro.fetch._http.cached_get`, which sets a research-identifying User-Agent, defaults to a 60-second timeout, and mirrors each URL under `data/raw/` alongside a JSON manifest. The archive-backed vintage providers call `fetch_with_backoff`, which retries rate-limit and transient-server codes on top of `puremacro._http` (User-Agent override, one-shot SSL fallback, 30-second default timeout) and stores each response body in the SQLite `http_cache` table, so every URL *they* touch shows up in `puremacro.cache.http_list_urls()`.
