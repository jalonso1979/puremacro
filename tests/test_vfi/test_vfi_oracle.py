...
ATOL, RTOL = 1e-8, 1e-7          # float64 backends (numba, cupy)
...
def test_mlx_matches_numpy_oracle():
    ...
    V_o, idx_o, *_ = solve_vfi(R_np, P, 0.95, tol=1e-10)
    R_mx = build_return_tensor(rf, a, z, params, xp=mx)
    V_m, idx_m, n_ap, n_it, sup = solve_vfi(R_mx, mx.array(P), 0.95, tol=1e-8, xp=mx)
    V_m_np = bk.to_numpy(V_m)
    idx_m_np = bk.to_numpy(idx_m)
    np.testing.assert_allclose(V_m_np, V_o, rtol=RTOL_F32, atol=ATOL_F32)
    np.testing.assert_array_equal(idx_m_np, bk.to_numpy(idx_o))
