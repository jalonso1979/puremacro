...
ax.set_ylabel("Net Sentiment Score", color=_nbstyle.TEXTO)
...
ax.bar(x - width, w_infra, width, label="Infrastructure (S-Curve)", color=_nbstyle.S1["color"], edgecolor=_nbstyle.SPINE)
ax.bar(x, w_trans, width, label="Direct Transfers (Front-Loaded)", color=_nbstyle.S2["color"], edgecolor=_nbstyle.SPINE)
ax.bar(x + width, w_tax, width, label="Tax Reform (Uniform)", color=_nbstyle.S3["color"], edgecolor=_nbstyle.SPINE)

ax.set_title("Empirical Policy Implementation Schedules (Weight Distribution)", fontsize=11, fontweight="bold")
ax.set_xlabel("Quarters Post-Announcement", color=_nbstyle.TEXTO)
ax.set_ylabel("Share of Total Expenditure / Impact", color=_nbstyle.TEXTO)
ax.set_xticks(x)
