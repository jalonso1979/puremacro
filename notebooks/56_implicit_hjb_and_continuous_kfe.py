...
# 4. **General Equilibrium Asset Market Clearing (Experiment 4):** The continuous Aiyagari general equilibrium root solve converges in a few hundredths of a second to market-clearing interest rate $r^* = 1.888\%$ ($0.018879$), real wage $w^* = 1.4495$ and aggregate capital $K^* = 6.2190$. Firm demand in the market-clearing panel is $K^d(r) = L^* \left( \alpha / (r + \delta) \right)^{1/(1-\alpha)}$ evaluated at the same aggregate labor supply the solver uses, $L^* = \sum_j z_j \pi_j = 0.6$; the notebook interpolates the sampled excess-supply curve $K^s(r) - K^d(r)$ and asserts that its zero sits within $5 \times 10^{-4}$ of $r^*$, so the plotted crossing and the dotted $r^*$ line agree. As predicted by macroeconomic theory, $r^* < \rho = 5.00\%$ because households accumulate precautionary buffer-stock savings against uninsurable idiosyncratic earnings risk, driving the equilibrium capital stock above the complete-markets level.
