...
# A simulated policy rate that goes non-positive would be both economically absurd
# and undefined under the log transform the revision tools apply to level series.
...
#
# 1. **Cross-Country Policy-Rate Paths (Experiment 1 & Figure 1):** Figure 1 plots the three simulated policy-rate paths at the latest snapshot. Mexico falls in a straight line from $11.25\%$ to $7.75\%$ ($25$ bp a quarter), Brazil from $12.75\%$ to $5.75\%$ ($50$ bp a quarter) and Chile from $9.50\%$ down to a $3.00\%$ floor. These are invented lines chosen to give the cross-country plumbing something to draw; the printed `Lowest Policy Rate` confirms the floor keeps every simulated rate strictly positive, which a log-transformed revision test requires. Their levels, their ordering and their slopes carry no information about the TIIE, the Selic or the TPM.
...
# %% [markdown]
# **Prompts.**
# 1. *Basic:* Modify `as_of_custom` to an earlier snapshot (e.g., `"2024-06-01"`). Observe how the cross-section shrinks to the information set that would have been on hand at that capture. Set it earlier than the first snapshot (`"2023-06-01"`) and the cell clamps it and says so, because before the first capture there is no information set to report.
