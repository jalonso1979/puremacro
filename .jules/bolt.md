...
## 2026-09-15 - Pandas iteration overhead
**Learning:** Using `iterrows()` to iterate over DataFrames causes significant overhead due to boxing every cell into a Python object, which becomes a major bottleneck for operations like quarterly expansion (e.g. 10x slower on 10k rows).
**Action:** Avoid `iterrows()`. For transformations that map cleanly to columns, use vectorized operations (e.g., `pd.concat` and `np.where`). For row-by-row iteration with complex dictionary-style access, use `.to_dict('records')` instead.
